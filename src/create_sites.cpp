 ...
 */
    ...
    for (int m = 0; m < 2; m++) { //for regular BCC sites
      ...
      offsets_3d(m,basis,0.5*xlattice,0.5*xlattice,6,&cmap[m][8]); //find Oct neighbors
      offsets_3d(m,basis,sqrt(5.0)/4.0*xlattice,sqrt(5.0)/4.0*xlattice,
		 24,&cmap[m][14]); //find Tet neighbors
    }
    for (int m = 2; m < 8; m++) { //for Oct sites
      offsets_3d(m,basis,1/2.0*xlattice,1/2.0*xlattice,
		 4,&cmap[m][0]); //find Oct neighbors
      offsets_3d(m,basis,0.25*xlattice,0.25*xlattice,
		 4,&cmap[m][4]); //find Tet neighbors
      offsets_3d(m,basis,1.0/2.0*xlattice,1.0/2.0*xlattice,
		 6,&cmap[m][8]); //find BCC neighbors
    }
    for (int m = 8; m < 20; m++) { //for Tet sites
      offsets_3d(m,basis,sqrt(2.0)/4.0*xlattice,sqrt(2.0)/4.0*xlattice,
		 4,&cmap[m][0]); // find Tet neighbors
      offsets_3d(m,basis,0.25*xlattice,0.25*xlattice,
		 2,&cmap[m][4]); // find Oct neighbors
      offsets_3d(m,basis,sqrt(5.0)/4.0*xlattice,sqrt(5.0)/4.0*xlattice,
		 4,&cmap[m][6]); // find BCC neighbors 
    }
/*
 *   printf("\n\n ----> DEBUG create_sites.cpp BCC_OCTA_TETRA \n\n");
 *    for (int m = 0; m < 20; m++){
...
//    printf("\n\n ----> DEBUG FCC_OCTA_TETRA in create_sites.cpp \n\n");
    for (int m = 0; m < 4; m++) {
      offsets_3d(m,basis,sqrt(2.0)/2.0*xlattice,sqrt(2.0)/2.0*xlattice,
		 12,&cmap[m][0]);
...
/*
 *   printf("\n\n ----> DEBUG create_sites.cpp FCC_OCTA_TETRA \n\n");
 *   for (int m = 0; m < 16; m++){
 *
 *     printf("\n START m %i",m);
 *     for (int n = 0; n < 26; n++){
 ...
 *        printf("\n START n %i", n);
 *       for (int o = 0; o < 4; o++){
 *               printf("\n %i, %i %i %i  cmap[m][n][o] m n o ", cmap[m][n][o], m, n, o);
 *       }
 *     printf("\n END n %i", n);
 *     }
 *     printf("\n END m %i", m);
...
//             The OCTA sites in BCC_OCTA_TETRA have 6 nearest neighbors with 2 in regular BCC sites.
//	       However since these two are not OCTA site neigbhors,they are not indexed in neighbors.

	       if (latstyle == BCC_OCTA_TETRA && ibasis > 1 && ibasis < 8 && m < 2 && ntarget != 6) {
		 }

//             The TETRA sites in BCC_OCTA_TETRA have 4 nearest neighbors in regular BCC sites.
//	       But, TETRA sites also have OCTA neighbors at the same distance as BCC sites, so don't index them.

	       else if (latstyle == BCC_OCTA_TETRA && ibasis > 7 && ibasis < 20 && m < 2) {
		 }
	       else {
	          cmapone[n][0] = i;
	          cmapone[n][1] = j;
